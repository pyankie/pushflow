...
export class DispatcherService implements OnModuleInit {
  ...
  private async handleTopicSubscribe(rawMessage: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to parse subscribe payload: ${rawMessage}`);
      return;
    }
  ...
  private async handleTopicUnsubscribe(rawMessage: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to parse unsubscribe payload: ${rawMessage}`);
      return;
    }
  ...
  private async handleTopicQuery(rawMessage: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to parse topic query payload: ${rawMessage}`);
      return;
    }
    ...
    try {
      const topics = await this.subscriptionService.getSubscriptions(
        receiverId
      );
      const publisher = this.redisService.getPublisher();
      await publisher.publish(
